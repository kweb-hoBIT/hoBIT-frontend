...
const Header: React.FC = () => {
	...
	return (
		<div className="h-[70px] bg-white fixed top-0 w-full border-b-2 border-[#bbbbbb] flex items-center px-[20px] py-[20px]">
			<div>
				<LuMenu
			...
			<div className="ml-auto relative w-[88px] h-[32px] bg-[#D9D9D9] rounded-[20px] flex items-center">
				<div
					className={`absolute h-full rounded-full justify-center transition-transform duration-500 ease-in-out bg-black ${isKorean ? 'translate-x-0 w-[50px]' : 'translate-x-[48px] w-[40px]'
						}`}
				></div>
				...
				<button
					...
					className={`w-[38px] h-full z-10 text-[16px] font-6semibold rounded-full transition-colors duration-500 ${!isKorean ? 'text-white' : 'text-[#aaaaaa]'
						}`}
				>
					EN
				</button>
