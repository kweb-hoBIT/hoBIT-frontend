		...
		<div>
			...
			{!currentCategory && !currentSeniorCategory && (
				<>
					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 mt-[10px]">
						{categories.map((category, index) => (
							<div
								key={index}
								className="bg-gray-100 min-h-[80px] flex flex-col items-center justify-center rounded-[20px] px-[10px] py-[5px] hover:bg-gray-200 cursor-pointer w-full"
					...
					<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 mt-[10px]">
						{seniorCategories.map((category, index) => (
							<div
								key={index}
								className="bg-[#FFEFEF] min-h-[80px] flex flex-col items-center justify-center rounded-[20px] px-[10px] py-[5px] hover:bg-[#FDDDDD] cursor-pointer w-full"
							...
							{faqTree?.tree.get(currentCategory) &&
								Array.from(faqTree.tree.get(currentCategory)!.keys()).map(
									(subCategory: Category, index: number) => (
										<div
											key={index}
											className="hover:bg-gray-200 bg-gray-100 min-h-[80px] flex items-center justify-center px-[10px] py-[5px] rounded-[20px] cursor-pointer"
			...
			{currentCategory &&
				...
				!currentSeniorSubCategory && (
					<>
						...
						<div className="bg-gray-100 rounded-[20px] px-[20px] py-[10px] mt-[10px] w-full max-w-md">
										...
										return (
											...
											>
												<button
													...
													className="font-5medium text-left text-[#686D76] text-lg md:text-xl inline-block py-[5px] rounded-[20px] mr-[5px] hover:text-black break-keep"
												>
													{isKorean ? faq.question_ko : faq.question_en}
												</button>
											</div>
