              ...
              >
                {answers.map((item: any, itemIndex: number) => {
                  const cardContent = (
                    <div
                      key={itemIndex}
                      className={`h-fit bg-gray-100 font-5medium text-base md:text-lg mt-[10px] rounded-[20px] px-[20px] py-[15px] w-full max-w-[330px] md:max-w-none md:w-[350px] break-words inline-block ${
                        itemIndex !== answers.length - 1 ? 'md:mr-[10px]' : ''
                      }`}
                    >
                      {itemIndex === 0 && (
                  ...
                  if (itemIndex === answers.length - 1) {
                    return (
                      <div key={itemIndex} className="flex flex-col">
                        {cardContent}
                        <Survey id={faq.id} user_question={sentValue} />
                      </div>
                    );
                  }
                  return cardContent;
